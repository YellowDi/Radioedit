...
local function PlaceCurrentFrame()
	...
	local points = ARM_CDB["FramePoints"][CurrentFrame]
	f:ClearAllPoints()
...
local function Reskinbox(box, name, value, ...)
	...
	box:SetScript("OnShow", function(self)
		if CurrentFrame ~= "NONE" then
			self:SetText(ARM_CDB["FramePoints"][CurrentFrame][value])
		else
			self:SetText("")
		end
	end)
	...
	box:SetScript("OnEnterPressed", function(self)
		if CurrentFrame ~= "NONE" then
			ARM_CDB["FramePoints"][CurrentFrame][value] = self:GetText()
			PlaceCurrentFrame()
		else
			self:SetText("")
		end
	...
	for k, v in pairs(anchors) do
		...
		frame[k]:SetScript("OnClick", function(self)
			if self:GetChecked() then
				ARM_CDB["FramePoints"][CurrentFrame][value] = k
				PlaceCurrentFrame()
			end
		end)
	end
	...
	for k, v in pairs(anchors) do
		frame[k]:HookScript("OnClick", function(self)
			for key, _ in pairs(anchors) do
				frame[key]:SetChecked(ARM_CDB["FramePoints"][CurrentFrame][value] == key)
			end
		end)
	end
...
local function DisplayCurrentFramePoint()
	local points = ARM_CDB["FramePoints"][CurrentFrame]
	for k, v in pairs(anchors) do
		Point1buttons[k]:SetChecked(ARM_CDB["FramePoints"][CurrentFrame]["a1"] == k)
		Point2buttons[k]:SetChecked(ARM_CDB["FramePoints"][CurrentFrame]["a2"] == k)
	end
...
ResetButton:SetScript("OnClick", function()
	if CurrentFrame ~= "NONE" then
		...
		ARM_CDB["FramePoints"][CurrentFrame].a1 = frame["point"].a1
		ARM_CDB["FramePoints"][CurrentFrame].parent = frame["point"].parent
		ARM_CDB["FramePoints"][CurrentFrame].a2 = frame["point"].a2
		ARM_CDB["FramePoints"][CurrentFrame].x = frame["point"].x
		ARM_CDB["FramePoints"][CurrentFrame].y = frame["point"].y
...
function T.CreateDragFrame(frame)
	...
	frame.df:SetScript("OnDragStop", function(self) 
		...
		ARM_CDB["FramePoints"][fname].x = ARM_CDB["FramePoints"][fname].x + x1
		ARM_CDB["FramePoints"][fname].y = ARM_CDB["FramePoints"][fname].y + y1
...
T.PlaceAllFrames = function()		
	for i = 1, #G.dragFrameList do
		...
		if ARM_CDB["FramePoints"][name] == nil then
			ARM_CDB["FramePoints"][name] = frame.point
		else
			if ARM_CDB["FramePoints"][name]["a1"] == nil then
				ARM_CDB["FramePoints"][name]["a1"] = frame.point.a1
			end
			if ARM_CDB["FramePoints"][name]["a2"] == nil then
				ARM_CDB["FramePoints"][name]["a2"] = frame.point.a2
			end
			if ARM_CDB["FramePoints"][name]["parent"] == nil then
				ARM_CDB["FramePoints"][name]["parent"] = frame.point.parent
			end
			if ARM_CDB["FramePoints"][name]["x"] == nil then
				ARM_CDB["FramePoints"][name]["x"] = frame.point.x
			end
			if ARM_CDB["FramePoints"][name]["y"] == nil then
				ARM_CDB["FramePoints"][name]["y"] = frame.point.y
			end
		end
		
		local points = ARM_CDB["FramePoints"][name]
...
T.ResetAll = function()
	for i = 1, #G.dragFrameList do
		...
		ARM_CDB["FramePoints"][f:GetName()] = {}
		for k, v in pairs (f.point) do
			ARM_CDB["FramePoints"][f:GetName()][k] = v
		end
