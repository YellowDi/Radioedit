...
local function DebugMessage(msg)
    if (not EXTVENDOR.DebugMode) then return; end
...
function ExtVendor_OnUpdate(self, elapsed)
    ...
    if (QUEUE_QUICKVENDOR_UPDATE) then
        ExtVendor_StartQuickVendorRefresh();
        QUEUE_QUICKVENDOR_UPDATE = false;
    elseif (EXTVENDOR.RefreshingQuickVendorList) then
...
function ExtVendor_Setup()
    ...
    ExtVendor_CheckSetting("quickvendor_msg_compact", true);
            ...
            if (name) then
                ...
                if (string.len(search) > 0) then
                    if (not string.find(string.lower(name), string.lower(search), 1, true)) then
                        isFiltered = true;
                        whyFiltered = L["FILTER_REASON_SEARCH_FILTER"];
                    end
                end
                -- check quality filter
                if (EXTVENDOR.SelectedQuality > 0) then
                    if ((quality < EXTVENDOR.SelectedQuality) or ((quality > EXTVENDOR.SelectedQuality) and EXTVENDOR.SpecificQuality)) then
                        isFiltered = true;
                        whyFiltered = L["FILTER_REASON_QUALITY_FILTER"];
                    end
                end
                ...
                if (EXTVENDOR_DATA['config']['usable_items'] and ((not isPurchasable) or ((not isUsable) and (not isDarkmoonReplica) and (not isHeirloom)))) then
                    isFiltered = true;
                    whyFiltered = L["FILTER_REASON_NOT_USABLE"];
                end
                ...
                if (EXTVENDOR_DATA['config']['optimal_armor'] and (not EXTVENDOR_DATA['config']['show_suboptimal_armor'])) then
                    if ((quality ~= 7) and isUsable and (not isDarkmoonReplica)) then
                        if (not ExtVendor_IsOptimalArmor(itemType, itemSubType, itemEquipLoc)) then
                            isFiltered = true;
                            whyFiltered = L["FILTER_REASON_SUBOPTIMAL"];
                        end
                    end
                end
                -- check slot filter
                if (SLOT_FILTER_INDEX > 0) then
                    if (SLOT_FILTERS[SLOT_FILTER_INDEX]) then
                        ...
                        if (not validSlot) then
                            isFiltered = true;
                            whyFiltered = L["FILTER_REASON_SLOT_FILTER"];
                        end
                    end
                end
                ...
                if (EXTVENDOR_SHOW_TRANSMOG_ONLY) then
                    if (not ExtVendor_ItemIsValidForTransmog(itemType, itemSubType, itemEquipLoc)) then
                        isFiltered = true;
                        whyFiltered = L["FILTER_REASON_NOT_TRANSMOG"];
                    end
                end
        ...
        if (index <= visibleMerchantItems) then
            ...
            if (name ~= nil) then
                ...
                if (itemButton.link) then
                    ...
                    __, __, quality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, __, itemSellPrice, __, __, bindType = unpackItemInfo(itemInfo[indexes[index]]);
                end
                ...
                if (not EXTVENDOR_DATA['config']['hide_filtered']) then
                    ...
                    -- check usability filter
                    if (EXTVENDOR_DATA['config']['usable_items'] and ((not isPurchasable) or ((not isUsable) and (not isHeirloom) and (not isDarkmoonReplica)))) then
                        isFiltered = true;
                    end
                    -- check quality filter
                    if (EXTVENDOR.SelectedQuality > 0) then
