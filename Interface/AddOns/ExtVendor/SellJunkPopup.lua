...
function ExtVendor_SellJunkProgressPopup_OnUpdate(self, elapsed)
    ...
    if (totalItemsSold) then
        ExtVendor_SellJunkProgressPopup_MainProgressBar:SetValue(math.min(PROGRESS_MAX, totalItemsSold));
    end
...
function ExtVendor_ShowProgressPopup()
    ...
    PROGRESS_MAX = table.maxn(EXTVENDOR_PROCESS_JUNK_LIST);
