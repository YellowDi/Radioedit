...
local OLD_HIGHPERFORMANCE = false;
...
local OLD_QUICKVENDOR_OUTDATEDGEAR = false;
local OLD_QUICKVENDOR_OUTDATEDFOOD = false;
local OLD_QUICKVENDOR_COMPACTMESSAGE = false;
local OLD_SCALE = 1;
-- ***********************************************************
...
function ExtVendorConfig_OnLoad(self)
	...
	ExtVendorConfig_QuickVendorContainer_CompactChatMessageText:SetText(L["OPTION_QUICKVENDOR_COMPACTMESSAGE"]);
	ExtVendorConfig_QuickVendorContainer_CompactChatMessage.tooltip = L["OPTION_QUICKVENDOR_COMPACTMESSAGE_TOOLTIP"];
...
function ExtVendorConfig_StoreCurrentSettings()
    ...
    OLD_HIGH_PERFORMANCE = EXTVENDOR_DATA['config']['high_performance'];
    ...
    OLD_QUICKVENDOR_OUTDATEDGEAR = EXTVENDOR_DATA['config']['quickvendor_oldgear'];
    OLD_QUICKVENDOR_OUTDATEDFOOD = EXTVENDOR_DATA['config']['quickvendor_oldfood'];
    OLD_QUICKVENDOR_COMPACTMESSAGE = EXTVENDOR_DATA['config']['quickvendor_msg_compact'];
end
...
function ExtVendorConfig_Cancel()
    ...
    EXTVENDOR_DATA['config']['high_performance'] = OLD_HIGH_PERFORMANCE;
    ...
    EXTVENDOR_DATA['config']['quickvendor_oldgear'] = OLD_QUICKVENDOR_OUTDATEDGEAR;
    EXTVENDOR_DATA['config']['quickvendor_oldfood'] = OLD_QUICKVENDOR_OUTDATEDFOOD;
    EXTVENDOR_DATA['config']['quickvendor_msg_compact'] = OLD_QUICKVENDOR_COMPACTMESSAGE;
...
function ExtVendorConfig_CheckBox_OnClick(self, id)
    ...
    elseif (id == 2) then
		if (self:GetChecked()) then
		    EXTVENDOR_DATA['config']['high_performance'] = true;
		else
		    EXTVENDOR_DATA['config']['high_performance'] = false;
		end
    ...
    elseif (id == 25) then
		if (self:GetChecked()) then
		    EXTVENDOR_DATA['config']['quickvendor_oldgear'] = true;
		else
		    EXTVENDOR_DATA['config']['quickvendor_oldgear'] = false;
		end
        ExtVendor_RefreshQuickVendorButton();
    elseif (id == 26) then
		if (self:GetChecked()) then
		    EXTVENDOR_DATA['config']['quickvendor_oldfood'] = true;
		else
		    EXTVENDOR_DATA['config']['quickvendor_oldfood'] = false;
		end
        ExtVendor_RefreshQuickVendorButton();
    elseif (id == 27) then
		if (self:GetChecked()) then
		    EXTVENDOR_DATA['config']['quickvendor_msg_compact'] = true;
		else
		    EXTVENDOR_DATA['config']['quickvendor_msg_compact'] = false;
		end
	end
end
