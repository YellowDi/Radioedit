...
local function apply()
	...
	for i = 1,math.min(#groups,2) do
		ApplyToGroup(groups[i],"",tank,healer,true)
	end
...
local function hop()
	...
	for i=1,#results do
		local id, _, _, comment, _, _, _,
				_, _, _, _,
				_, leaderName, numMembers, isAutoAccept = C_LFGList_GetSearchResultInfo(results[i])
		if isAutoAccept and numMembers < 5 and filter_realm(leaderName) then
			groups[#groups+1] = id
		end
	end
	if 0 < #groups then
		if LookingForGroup.db.profile.hardware then
			local dialog = StaticPopupDialogs.LookingForGroup_CR_HardwareAPIDialog
			if dialog == nil then
				StaticPopupDialogs.LookingForGroup_CR_HardwareAPIDialog = {text = SIGN_UP,button1=ACCEPT,button2=CANCEL,OnAccept = apply,timeOut = 45}
			end
			StaticPopup_Show("LookingForGroup_CR_HardwareAPIDialog")
		else
			apply()
		end
	end
