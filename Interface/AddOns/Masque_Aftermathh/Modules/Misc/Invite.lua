...
if not (C.Misc.AutoAccpectInvites) then
	return
end
		...
		if (not UnitExists("party1") or UnitIsGroupLeader("player") or UnitIsGroupAssistant("player")) and strmatch(strlower(message), '^inv') then
			if event == "CHAT_MSG_WHISPER" then
				if Invite:GetQueueStatus() then
					SendChatMessage("Sorry! I'm currently in Queue!", "WHISPER", nil, sender)
				else
					InviteUnit(sender)
				end
			else
				...
				if Invite:GetQueueStatus() then
					BNSendWhisper(presenceID, "Sorry! I'm currently in Queue!")
				else
...
Invite:RegisterEvent("PLAYER_LOGIN")
Invite:RegisterEvent("PLAYER_ENTERING_WORLD")
Invite:RegisterEvent("ADDON_LOADED")
