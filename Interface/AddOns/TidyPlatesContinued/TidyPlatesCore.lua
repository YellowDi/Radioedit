...
do
	...
	function OnNewNameplate(plate, plateid)
		...
		local castbar = CreateTidyPlatesContStatusbar(extended)
		...
		-- Default Fonts
		visual.name:SetFontObject("TidyPlatesContFontNormal")
		visual.level:SetFontObject("TidyPlatesContFontSmall")
		visual.spelltext:SetFontObject("TidyPlatesContFontNormal")
		visual.customtext:SetFontObject("TidyPlatesContFontSmall")
...
do
	...
	TidyPlatesContCore:SetFrameStrata("TOOLTIP") 	-- When parented to WorldFrame, causes OnUpdate handler to run close to last
	TidyPlatesContCore:SetScript("OnEvent", EventHandler)
	for eventName in pairs(CoreEvents) do TidyPlatesContCore:RegisterEvent(eventName) end
end
...
TidyPlatesContInternal.UseTheme = UseTheme
...
local function GetThemeName()
	return TidyPlatesContOptions.ActiveTheme
end

TidyPlatesCont.GetTheme = GetTheme
TidyPlatesCont.GetThemeName = GetThemeName
...
function TidyPlatesCont:ActivateTheme(theme) if theme and type(theme) == 'table' then TidyPlatesCont.ActiveThemeTable, activetheme = theme, theme; ResetPlates = true; end end
function TidyPlatesCont.OverrideFonts( enable) OverrideFonts = enable; end

-- Old and needing deleting - Just here to avoid errors
function TidyPlatesCont:EnableFadeIn() EnableFadeIn = true; end
function TidyPlatesCont:DisableFadeIn() EnableFadeIn = nil; end
TidyPlatesCont.RequestWidgetUpdate = TidyPlatesCont.RequestUpdate
TidyPlatesCont.RequestDelegateUpdate = TidyPlatesCont.RequestUpdate
