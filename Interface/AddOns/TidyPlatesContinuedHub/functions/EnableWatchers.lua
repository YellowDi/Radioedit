...
--]]
	...
	if (ScaleFunctionsUniversal[LocalVars.ScaleSpotlightMode] == ScaleFunctionByEnemyHealer) or
		...
		TidyPlatesContUtility:DisableHealerTrack()
	end
	--]]
	...
	--		TidyPlatesContWidgets:EnableTankWatch()	-- Off-Tank support		-- Lots of shtuff depends on this.
	--else TidyPlatesContWidgets:DisableTankWatch() end

	-- 6.0 Disabled
	--if LocalVars.ColorShowPartyAggro then TidyPlatesContWidgets:EnableAggroWatch()	-- Group aggro holders
	--else TidyPlatesContWidgets:DisableAggroWatch() end
