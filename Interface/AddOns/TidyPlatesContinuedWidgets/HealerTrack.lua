...
-- Widget
---------------------------------------------------------------------------------------------------------
local widgetPath = "Interface\\Addons\\TidyPlatesContinuedWidgets\\HealerWidget"
...
TidyPlatesContWidgets.CreateHealerWidget = CreateWidgetFrame
...
local function UpdateRolesViaScoreboard()
	...
	if NumScores > 0 then
		...
		if UpdateIsNeeded then TidyPlatesCont:RequestDelegateUpdate() end
	end
		...
		if SpellEvents[combatevent] then								-- Filter: Specific spell events
			if HealerSpells[spellid] then								-- Filter: Known Healing Spells
				...
				if RoleList[rawName] ~= "Healer" then
					...
					TidyPlatesCont:RequestDelegateUpdate()
				end
			end
		end
	end
...
local function Enable()
	--print("TidyPlatesContWidgets.HealerTrack:Enable")
...
TidyPlatesContUtility.EnableHealerTrack = Enable
TidyPlatesContUtility.DisableHealerTrack = Disable
TidyPlatesContUtility.IsHealer = IsHealer
