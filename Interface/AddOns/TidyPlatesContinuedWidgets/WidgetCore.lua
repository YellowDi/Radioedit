...
do
	...
	function TidyPlatesContWidgets:HideIn(frame, expiration)
		-- Register Frame
...
do
	...
	local function CheckFramelist(self)
		...
		for frame, expiration in pairs(Framelist) do
			...
			if expiration < curTime then
				...
				-- frame:Hide()
				Framelist[frame] = nil
				--TidyPlatesCont:RequestDelegateUpdate()		-- Request an Update on Delegate functions, so we can catch when auras fall off
			-- If still active...
			else
				-- Update the frame
	...
	TidyPlatesContWidgets.PolledHideIn = PolledHideIn
end
...
TidyPlatesContWidgets.ResetWidgets = TidyPlatesCont.ResetWidgets
TidyPlatesContWidgets.EnableTankWatch = DummyFunction
TidyPlatesContWidgets.DisableTankWatch = DummyFunction
TidyPlatesContWidgets.EnableAggroWatch = DummyFunction
