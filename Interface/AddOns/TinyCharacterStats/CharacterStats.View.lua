...
CharacterStatsPane.ItemLevelFrame.Value:SetFont(CharacterStatsPane.ItemLevelFrame.Value:GetFont(), 14, "OUTLINE")
...
local ShownData = Addon:MergeTable({
...
do
    ...
    configButton:SetSize(20, 22)
    configButton:SetPoint("BOTTOMLEFT", PaperDollSidebarTab1, "BOTTOMLEFT", -1, -4)
    ...
    frame:SetScript("OnEvent", function(self, event, ...)
        ...
        if (TinyCharacterStatsDB[uniqueKey]) then
            if (ShownData.uniqueKey ~= uniqueKey) then
                ShownData = Addon:MergeTable(TinyCharacterStatsDB[uniqueKey])
            end
        else
